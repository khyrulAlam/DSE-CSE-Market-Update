...
async function kakkuGetCSEAll(){
	const url = CROSSURL+"?url="+"http://cse.com.bd/current_share_price_tc.php";
	const res = await fetch(url).then(response=>response.text());
	const html = await new DOMParser().parseFromString(res, 'text/html');
	const report   = await html.querySelector('#report');
	const tr   = await [...report.querySelectorAll('table tr')];
	const data = [];
